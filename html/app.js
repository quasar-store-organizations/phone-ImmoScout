window.addEventListener('message', e => {
    if (e.data === 'app-opened') {
        ...
        $('#owned-contentt').html(``)
        $.post('https://phone-ImmoScout/getHouses', {}, function (data) {
            if (!data) return
            for (let i = 0; i < Object.keys(data).length; i++) {
                let house = data[i]
                if (!house.owned) {
                    html += `<div class="custom-box">
                        <img class="house-tier" src="./img/shells/${house.tier}.png"></img>
                        <div class="nopadding-nomargin">
                            <p class="house-label">${house.label}</p>
                            <p><img class="option-icon" src="./img/distance.svg"></img> ${house.distance.toFixed(2)} m</p>
                            <p><img class="option-icon" src="./img/price.svg"></img> ${Number(house.price).toLocaleString("de-DE", { minimumFractionDigits: 2 })}</p>
                        </div>
...
$(document).on('click', '.custom-button', function (e) {
    let overid = $(this).attr("houseid")
    SendPhoneNotificationOld('immoScout', 'ImmoScout24', 'GPS updated!', 3000)
    $.post('https://phone-ImmoScout/setHousePosition', JSON.stringify({
        label: overid,
    }))
})

$(document).on('click', '#owned-houses', function (e) {
    $('#owned-contentt').show();
    $('#houses-contentt').hide();
})

$(document).on('click', '#all-houses', function (e) {
    $('#owned-contentt').hide();
    $('#houses-contentt').show();
})